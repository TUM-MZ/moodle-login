...
class FormHandler(object, metaclass = FormHandlerMeta):
...
class DS(FormHandler):
    ...
    def submit(self, opener, res):
        ...
        data = urllib.parse.urlencode(wayf_data).encode()
...
class WAYF(FormHandler):
    ...
    def submit(self, opener, res):
        ...
        idp = self.idp
        data = self.data
        if idp.get_idp() not in data['origin']:
...
class IdPSPForm(FormHandler):
    ...
    def submit(self, opener, res):
        ...
        data = self.data
        url = urlparse.urljoin(res.url, data['form']['action'])
        data = urllib.parse.urlencode({'SAMLResponse':data['SAMLResponse']['value'], 'TARGET':'cookie'}).encode()
        request = urllib.request.Request(url, data=data)
        log.debug("POST: %s?%s" % (request.get_full_url(), data.decode()))
        response = opener.open(request)
        return request, response
...
class IdPSPFormRelayState(FormHandler):
    ...
    def submit(self, opener, res):
        ...
        data = self.data
        url = urlparse.urljoin(res.url, data['form']['action'])
        data = urllib.parse.urlencode({'SAMLResponse':data['SAMLResponse']['value'], 'RelayState':'cookie'}).encode()
        request = urllib.request.Request(url, data=data)
